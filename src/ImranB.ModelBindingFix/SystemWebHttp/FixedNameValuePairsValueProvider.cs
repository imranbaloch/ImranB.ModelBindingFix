...
{
    public class FixedNameValuePairsValueProvider : IEnumerableValueProvider
    {
        ...
        public FixedNameValuePairsValueProvider(IEnumerable<KeyValuePair<string, string>> values, CultureInfo culture)
            : this(() => values, culture)
        {
            if (values == null)
            {
        ...
        [SuppressMessage("Microsoft.Design", "CA1006:DoNotNestGenericTypesInMemberSignatures", Justification = "Represents a collection of name/value pairs, cannot use NameValueCollection because it performs poorly")]
        public FixedNameValuePairsValueProvider(Func<IEnumerable<KeyValuePair<string, string>>> valuesFactory, CultureInfo culture)
        {
            ...
            _culture = culture;
            _prefixContainer = new Lazy<FixedPrefixContainer>(() => new FixedPrefixContainer(_values.Value.Keys), isThreadSafe: true);
        }
